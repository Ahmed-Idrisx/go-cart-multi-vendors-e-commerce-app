...
export default function Footer() {
  ...
  const linkSections = [
    ...
    },
    {
      ...
      links: [
        {
          text: "ahmedidrisx@gmail.com",
          path: "mailto:ahmedidrisx@gmail.com",
          icon: MailIcon,
        },
        { text: "+201015041410", path: "tel:+201015041410", icon: PhoneIcon },
        { text: "Cairo Egypt", path: "/", icon: MapPinIcon },
      ],
    },
  ];
  ...
  const socialIcons = [
    ...
    {
      icon: LinkedinIcon,
      link: "https://www.linkedin.com/in/ahmed-idris-68b4233ba/",
    },
  ];

  return (
    <footer className=" bg-white dark:bg-slate-900 border-t border-gray-250 dark:border-slate-800 transition-colors duration-200">
      <div className="max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row items-start justify-between gap-10 px-2 py-10 border-b border-slate-500/30 text-slate-500 dark:text-slate-400">
          <div>
            <Link
              href="/"
            ...
            <p className="max-w-102.5 mt-6 text-sm">
              Welcome to Go Cart, your ultimate destination for the latest and
            ...
            <div className="flex items-center gap-3 mt-5">
              {socialIcons.map((item, i) => (
                <Link
                  href={item.link}
                  key={i}
                  className="flex items-center justify-center w-10 h-10 bg-slate-100 dark:bg-slate-800 hover:scale-105 hover:border border-slate-300 dark:border-slate-700 transition rounded-full text-slate-600 dark:text-slate-300"
                >
                  <item.icon />
                </Link>
              ))}
          ...
          <div className="flex flex-wrap justify-between w-full md:w-[45%] gap-5 text-sm">
            {linkSections.map((section, index) => (
              <div key={index}>
                <h3 className="font-semibold text-slate-750 dark:text-slate-200 md:mb-5 mb-3">
                  {section.title}
                </h3>
                <ul className="space-y-2.5">
                  {section.links.map((link, i) => (
                    <li key={i} className="flex items-center gap-2">
                      ...
                      <Link
                        href={link.path}
                        className="hover:underline transition text-slate-500 dark:text-slate-400"
                      >
                        {link.text}
                      </Link>
                    </li>
